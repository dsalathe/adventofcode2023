...
import scala.collection.mutable.Map as HMap
...
object Part2DP {
    ...
    def longestPath(graph: Map[Position, Set[Edge]], startPosition: Position, endPosition: Position): Int =
        def traverse(position: Position, visited: Set[Position], cache: HMap[(Position, Set[Position]), Int]): Int = if cache.contains((position, visited)) then cache((position, visited)) else
            val neighbors = graph(position).filterNot(n => visited.contains(n.to))
            val totalDistance = neighbors.map(n =>
                if n.to == startPosition then
                    n.distance 
                else 
                    n.distance + traverse(n.to, visited + position, cache)
            ).maxOption.getOrElse(0)
            if totalDistance > 0 then {cache.put((position, visited), totalDistance)}
            totalDistance

        traverse(endPosition, Set(), HMap[(Position, Set[Position]), Int]())
