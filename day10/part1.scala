...
object Part1 {
    ...
    case class Node(position: Position, directions: Seq[Direction]):
        export position.*
    ...
    def parseInput(s: String): (Position, Graph) =
        ...
        val nodesWithCoveredStart = lines.zipWithIndex.flatMap {case (line, y) =>
            line.zipWithIndex.map {case (c, x) =>
                ...
                Node(position, directions)
            }
        }.map(node => node.position -> node).toMap

        val uncoveredStartDirections = uncoverStartDirection(nodesWithCoveredStart, startPosition)
    ...
    def uncoverStartDirection(nodes: Map[Position, Node], startPosition: Position): Seq[Direction] =
